...
from tensorflow.keras.layers import Activation, Dense, LSTM, Dropout, \
    Lambda, Input, Multiply, Layer, Conv1D, Dense, GaussianNoise
...
class DTLN_model():
    ...
    def seperation_kernel(self, num_layer, mask_size, x, stateful=False):
        ...
        for idx in range(num_layer):
            x = LSTM(self.numUnits, return_sequences=True, stateful=stateful, name='new_layer'+str(idx))(x)
            # using dropout between the LSTM layer for regularization 
            if idx<(num_layer-1):
                x = Dropout(self.dropout, name='new_layer_2')(x)

        # creating the mask with a Dense and an Activation layer
        mask = Dense(mask_size, name='new_layer_3')(x)
        mask = Activation(self.activation, name='new_layer_4')(mask)
        # returning the mask
        return mask

    def seperation_kernel2(self, num_layer, mask_size, x, stateful=False):
        '''
        Method to create a separation kernel.
        !! Important !!: Do not use this layer with a Lambda layer. If used with
        a Lambda layer the gradients are updated correctly.
        ...
        # creating num_layer number of LSTM layers
        for idx in range(num_layer):
            x = LSTM(self.numUnits, return_sequences=True, stateful=stateful, name='new_layer_'+str(idx))(x)
            # using dropout between the LSTM layer for regularization
            if idx<(num_layer-1):
                x = Dropout(self.dropout, name='new_layer_2_')(x)

        # creating the mask with a Dense and an Activation layer
        mask = Dense(mask_size, name='new_layer_3_')(x)
        mask = Activation(self.activation, name='new_layer_4_')(mask)
        # returning the mask
        return mask
    ...
    def build_DTLN_model(self, norm_stft=False):
        ...
        # calculate STFT
        mag,angle = Lambda(self.stftLayer, name='layer1')(time_dat)
        # normalizing log magnitude stfts to get more robust against level variations
        if norm_stft:
            mag_norm = InstantLayerNormalization(name='layer2')(tf.math.log(mag + 1e-7))
        else:
        ...
        estimated_mag = Multiply(name='layer3')([mag, mask_1])
        # transform frames back to time domain
        estimated_frames_1 = Lambda(self.ifftLayer, name='layer4')([estimated_mag,angle])
        # encode time domain frames to feature domain



        encoded = Dense(512, activation='relu', name="encoder_layer1")(estimated_frames_1)
        encoded1 = Dense(512, activation='linear', name="encoder_layer2")(encoded)
        encoded2 = BatchNormalization()(encoded1, name="encoder_layer3")

        EbNo_train = 5.01187  # coverted 7 db of EbNo
        encoded3 = GaussianNoise(np.sqrt(1 / (2 * (4/7) * EbNo_train)))(encoded2)

        decoded = Dense(512, activation='relu', name="encoder_layer4")(encoded3)
        decoded1 = Dense(512, activation='softmax', name="encoder_layer5")(decoded)



        encoded_frames = Conv1D(self.encoder_size,1,strides=1,use_bias=False, name='layer5')(estimated_frames_1)
        # normalize the input to the separation kernel
        encoded_frames_norm = InstantLayerNormalization(name='layer6')(encoded_frames)
        # predict mask based on the normalized feature frames
        mask_2 = self.seperation_kernel2(self.numLayer, self.encoder_size, encoded_frames_norm)
        # multiply encoded frames with the mask
        estimated = Multiply(name='layer7')([encoded_frames, mask_2])
        # decode the frames back to time domain
        decoded_frames = Conv1D(self.blockLen, 1, padding='causal',use_bias=False, name='layer8')(estimated)
        # create waveform with overlap and add procedure
        estimated_sig = Lambda(self.overlapAddLayer, name='layer9')(decoded_frames)
    ...
    def build_DTLN_model1(self, norm_stft=False):
        '''
        Method to build and compile the DTLN model. The model takes time domain
        batches of size (batchsize, len_in_samples) and returns enhanced clips
        in the same dimensions. As optimizer for the Training process the Adam
        ...
        # create the model
        self.model = Model(inputs=time_dat, outputs=mag_norm)



    def build_DTLN_model2(self, norm_stft=False):
        '''
        Method to build and compile the DTLN model. The model takes time domain
        batches of size (batchsize, len_in_samples) and returns enhanced clips
        in the same dimensions. As optimizer for the Training process the Adam
        ...
        # input layer for time signal
        time_dat = Input(batch_shape=(None, None, 512))

        # encode time domain frames to feature domain
        encoded_frames = Conv1D(self.encoder_size, 1, strides=1, use_bias=False, name='layer5')(time_dat)
        # normalize the input to the separation kernel
        encoded_frames_norm = InstantLayerNormalization(name='layer6')(encoded_frames)
        # predict mask based on the normalized feature frames
        mask_2 = self.seperation_kernel2(self.numLayer, self.encoder_size, encoded_frames_norm)
